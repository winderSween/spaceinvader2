...
/**
 * 
 * @author This class represents the level
 */
public class BossLevel {
	private BossLevelConf conf;
	private int grids[][];
	...
	public BossLevel(BossLevelConf conf) {
		this.conf = conf;
		...
		//int begin = width / 2;
		// init boss
		String ps = this.conf.getPoints();
		String pp[] = ps.split(",");
		for (int i = 0; i < pp.length - 1; i = i + 2) {
			int x = Integer.parseInt(pp[i]);
			int y = Integer.parseInt(pp[i + 1]);
			grids[x][y] = 1;
		}
	...
	public void alienFire() {
		for (int i = 0; i < conf.getWidth(); i++) {
			...
			if (maxRow != -1) {
				...
				if (rand < 40) {
					try {
						if (grids[maxRow + conf.getDown_step() + 1][i] != 3) {
							grids[maxRow + conf.getDown_step() + 1][i] = 5;
	...
	public boolean moveAliens() {
		...
		for (int i = 0; i < conf.getWidth(); i++) {
			for (int k = conf.getHeight() - 1; k > 0; k--) {
				try {
					if (grids[k - conf.getDown_step()][i] == 1) {
						grids[k - conf.getDown_step()][i] = 0;
						grids[k][i] = 1;
					} else {
						if (grids[k][i] == 1
								&& grids[k - conf.getDown_step()][i] == 0) {
			...
			for (int j = 0; j < conf.getHeight(); j++) {
				if (grids[j][i] == 4) {
					...
					for (int k = j; k >= end; k--) {
						...
						if (grids[k][i] == 1) {
							alienKilled = true;
							conf.setBoss_lives(conf.getBoss_lives() - 1);
							break;
						}
					}
					if (!alienKilled) {
